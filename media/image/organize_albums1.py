...
class for global variables."""
    ...
    m4a_path = os.path.join(album_folder, f"{album_name}.m4a")
 ...
 try:
  ...
  raise
    ...
    for file in files:
        ...
        elif file.endswith(".m4a") and not os.path.exists(m4a_path):
            shutil.move(file_path, m4a_path)
            logger.info(f"Moved: {file} to {m4a_path}")
        elif file.endswith(".mp4") and not os.path.exists(mp4_path):
