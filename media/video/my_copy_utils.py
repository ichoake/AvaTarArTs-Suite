...
class for global variables."""
    ...
    backup_path = os.path.join(backup_root, os.path.basename(file_path))
...
async def ensure_dir(directory):
def ensure_dir(directory): -> Any
 try:
...
with open(csv_file_path, newline="") as csvfile:
    for row in filereader:
        ...
        logger.info(f"Copied file to destination: {file_path} to {destination_path}")

        # Copy the file to the backup location
        shutil.copy2(file_path, backup_path)
        logger.info(f"Copied file to backup: {file_path} to {backup_path}")
