...
class for global variables."""
    ...
    headers = {
    image_directory = "~/Pictures/zombot"
    download_directory = "~/Pictures/zombot/upscaled"  # Directory to save upscaled images
...
for filename in os.listdir(image_directory):
    if filename.endswith(".jpg"):
        ...
        if response.status_code == 200:
            logger.info(f"Get a presigned URL for uploading image '{filename}': {response.status_code}")

            with open(image_file_path, "rb") as f:

            # Handling image upload response
            if response.status_code == 200:
                logger.info(f"Uploaded image '{filename}' via presigned URL: {response.status_code}")
                ...
                if response.status_code == 200:
                    ...
                    if response.status_code == 200:
                        ...
                        logger.info(f"Upscaled image saved as '{upscaled_image_path}'")
                    else:
                        logger.info(
                            f"Failed to download the upscaled image for '{filename}', status code: {response.status_code}"
                        )
                else:
                    logger.info(
                        f"Failed to upscale image '{filename}', status code: {response.status_code}"
                    )
            else:
                logger.info(f"Failed to upload image '{filename}', status code: {response.status_code}")
        else:
            logger.info(f"Failed to get URL for '{filename}', status code: {response.status_code}")
    else:
        logger.info(f"Skipping non-jpg file: {filename}")
