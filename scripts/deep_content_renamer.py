    ...
    def deep_analyze(self, filepath: Path) -> Dict:
        ...
        try:
            ...
            if analysis['docstring']:
                ...
                analysis['main_purpose'] = first_sentence[:100]
...
class DeepContentRenamer:
    ...
    def __init__(self, target_dir: str, dry_run: bool = True, limit: int = 500):
    ...
    def scan_and_analyze(self):
        ...
        print(f"{Colors.GREEN}Analyzing {len(python_files)} Python files...{Colors.END}\n")

        needs_rename = [f for f in python_files if self.should_rename(f)]

        print(f"{Colors.YELLOW}Files needing better names: {len(needs_rename)}{Colors.END}\n")

        # Deep analyze each file
        for idx, filepath in enumerate(needs_rename[:self.limit], 1):
            if idx % 50 == 0:
                print(f"{Colors.YELLOW}Analyzing: {idx}/{min(len(needs_rename), self.limit)}...{Colors.END}", end='\r')
        ...
        print(f"{Colors.CYAN}Files to rename: {len(self.rename_plan)}{Colors.END}")

    def execute_renames(self):
        """Execute the rename plan"""

        print(f"\n{Colors.CYAN}{Colors.BOLD}{'='*80}")
        print(f"🏷️ EXECUTING INTELLIGENT RENAMES")
        print(f"{'='*80}{Colors.END}\n")

        print(f"{Colors.YELLOW}Total: {len(self.rename_plan)}{Colors.END}")
        ...
        with open(csv_file, 'w', newline='') as f:
            ...
            for item in self.rename_plan:
                writer.writerow([
                    ...
                    item['purpose'][:100] if item['purpose'] else '',
                    item['parent'],
    ...
    def run(self):
        ...
        print("╔═══════════════════════════════════════════════════════════════════════════════╗")
        print("║                                                                               ║")
        print("║           🧠 DEEP CONTENT-AWARE INTELLIGENT RENAMER 🚀                       ║")
        print("║                                                                               ║")
        print("║     Understands What Code Does Before Renaming                               ║")
        print("║                                                                               ║")
        print("╚═══════════════════════════════════════════════════════════════════════════════╝")
        print(f"{Colors.END}\n")

        print(f"{Colors.CYAN}Target: {self.target_dir}{Colors.END}")
        print(f"{Colors.CYAN}Limit: {self.limit} files (for safety){Colors.END}")

        if self.dry_run:
            print(f"{Colors.YELLOW}MODE: DRY RUN{Colors.END}\n")
        else:
            print(f"{Colors.RED}MODE: LIVE RENAME{Colors.END}\n")
        ...
        print(f"  To Rename: {Colors.CYAN}{len(self.rename_plan):,}{Colors.END}")
        print(f"  Renamed: {Colors.CYAN}{self.stats['renamed']:,}{Colors.END}\n")
...
def main():
    ...
    parser.add_argument('--limit', type=int, default=500, help='Max files to rename')
