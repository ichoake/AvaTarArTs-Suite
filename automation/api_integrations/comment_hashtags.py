...
if len(sys.argv) < 3:
    logger.info("USAGE: Pass a path to the file with comments " "and a hashtag to comment")
    logger.info("Example: %s comments_emoji.txt dog cat" % sys.argv[0])
    exit()

if not os.path.exists(comments_file_name):
    logger.info("Can't find '%s' file." % comments_file_name)
    exit()
