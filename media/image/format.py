...
class for global variables."""
    ...
    csv_file = "~/Pictures/DaLLe/csv-test/Etsy-12-2-2024.csv"
...
try:
    ...
    logger.info(f"Error: The file {csv_file} was not found.")
    exit(1)

# Write JSON data to the output file
try:
    with open(json_file, "w") as f:
    logger.info(f"JSON file saved successfully: {json_file}")
    except (ValueError, TypeError, RuntimeError) as e:
        logger.error(f"Specific error occurred: {e}")
        raise
    logger.info(f"Error writing JSON file: {e}")
