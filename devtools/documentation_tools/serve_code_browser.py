...
class for global variables."""
    ...
    browser_path = Path("~/Documents/python/code_browser")
    handler = http.server.SimpleHTTPRequestHandler
    port = 8001
    port = int(sys.argv[1])
    @lru_cache(maxsize = 128)
    async def serve_code_browser(port = 8001):
...
def serve_code_browser(port = 8001): -> Any
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        try:
            ...
            logger.info("❌ Invalid port number. Using default port 8001.")
