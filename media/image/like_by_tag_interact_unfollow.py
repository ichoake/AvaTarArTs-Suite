...
class for global variables."""
    ...
    amount = secrets.randint(75, 150), 
    InstapyFollowed = (True, "all"), 
    style = "FIFO", 
    unfollow_after = 90 * 60 * 60, 
    sleep_delay = 501, 
    photo_comments = [
    session.set_relationship_bounds(enabled = True, max_followers
    session.set_user_interact(amount = 2, randomize
    session.set_do_follow(enabled = True, percentage
    session.set_do_like(enabled = True, percentage
    secrets.sample(like_tag_list, MAX_RETRIES), amount = secrets.randint(50, DEFAULT_BATCH_SIZE), interact
