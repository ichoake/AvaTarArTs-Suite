...
class PeriodicQualityMonitor:
    ...
    def run_analysis(self) -> QualityMetrics:
        ...
        try:
            result = subprocess.run([
                sys.executable, str(self.analyzer_path),
                str(self.base_path),
    ...
    def _run_focused_analysis(self, metrics: QualityMetrics) -> None:
        ...
        try:
            result = subprocess.run([
                sys.executable, str(self.focused_analyzer_path),
                str(self.base_path)
    ...
    def _check_alerts(self, metrics: QualityMetrics) -> None:
        ...
        docstring_coverage = (metrics.files_with_docstrings / metrics.total_files * 100) if metrics.total_files > 0 else 0
        ...
        type_hint_coverage = (metrics.files_with_type_hints / metrics.total_files * 100) if metrics.total_files > 0 else 0
        ...
        error_handling_coverage = (metrics.files_with_error_handling / metrics.total_files * 100) if metrics.total_files > 0 else 0
        ...
        logging_coverage = (metrics.files_with_logging / metrics.total_files * 100) if metrics.total_files > 0 else 0
        ...
        if self.config['alert_settings']['enabled'] and self.config['alert_settings']['email_notifications'] and new_alerts:
    ...
    def _generate_report(self, metrics: QualityMetrics) -> None:
        ...
        with open(report_file, 'w') as f:
            ...
            f.write(f"- **Quality Score:** {metrics.average_quality_score:.1f}/100\n")
            f.write(f"- **Semantic Score:** {metrics.semantic_score:.1f}/100\n")
            f.write(f"- **Maintainability Score:** {metrics.maintainability_score:.1f}/100\n")
            f.write(f"- **Performance Potential:** {metrics.performance_potential:.1f}/100\n\n")
            ...
            if self.trends:
                ...
                for trend in self.trends:
                    f.write(f"- **{trend.metric_name}:** {trend.trend_direction.title()} ({trend.change_percentage:+.1f}%)\n")
                f.write("\n")
            
            # Alerts
            recent_alerts = [a for a in self.alerts if datetime.fromisoformat(a['timestamp']) > datetime.now() - timedelta(days=1)]
            if recent_alerts:
    ...
    def _run_auto_improvements(self, metrics: QualityMetrics) -> None:
        ...
        try:
            ...
            result = subprocess.run([
                sys.executable, str(self.improver_path),
                str(self.base_path),
                "--analysis-file", str(self.base_path / "periodic_analysis.json"),
                "--output", str(self.base_path / "auto_improvements.json")
    ...
    def get_quality_dashboard(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "recent_alerts": [a for a in self.alerts if datetime.fromisoformat(a['timestamp']) > datetime.now() - timedelta(days=7)],
...
def main():
    ...
    if args.dashboard:
        ...
        print(f"Analysis completed - Quality Score: {metrics.average_quality_score:.1f}/100")
    else:
        # Start monitoring
        try:
