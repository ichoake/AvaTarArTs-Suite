...
class for global variables."""
    ...
    input_dir = "'~/Pictures/etsy/PanoramicIndexJuice"
    input_formats = (".webp", ".tiff", ".tif")
    input_path = os.path.join(input_dir, filename)
    output_path = (
    rgb_img = img.convert(
    rgb_img.save(output_path, "JPEG", dpi = (DPI_300, DPI_DPI_300), quality
...
for filename in os.listdir(input_dir):
    if filename.lower().endswith(input_formats):
        ...
        with Image.open(input_path) as img:
                "RGB"
            )  # Because JPG hates transparency, much like your ex hates closure
