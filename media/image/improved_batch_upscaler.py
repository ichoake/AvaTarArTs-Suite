...
class ImprovedBatchUpscaler:
    ...
    def optimize_file_size(self, image_path: Union[str, Path]) -> Tuple[bool, str]:
        ...
        logger.debug(f"Optimizing file size for {image_path} (current: {current_size / (1024*1024):.1f}MB)")
            ...
            if success and os.path.exists(temp_path):
                ...
                if temp_size <= max_size_bytes:
                    try:
                        ...
                        logger.debug(f"Optimized to {quality}% quality ({temp_size / (1024*1024):.1f}MB)")
        ...
        for image_path in progress_bar:
            ...
            if result.status == ProcessingStatus.SUCCESS:
                ...
                if HAS_TQDM:
                    ...
                    print(f"✅ {image_path.name} -> {result.file_size_mb:.1f}MB" if result.file_size_mb else f"✅ {image_path.name}")
            else:
                failed += 1
                if HAS_TQDM:
                    progress_bar.set_postfix({
...
def main():
    ...
    print("🖼️  IMPROVED BATCH IMAGE UPSCALER")
    print("=" * 50)
    print("Enhanced batch processing with progress tracking")
    print("Features: Error handling, progress bars, resume capability")
    print("=" * 50)
    ...
    print(f"\n💡 All images are:")
    print(f"  • {upscaler.config.target_dpi} DPI for print quality")
    print(f"  • Under {upscaler.config.max_file_size_mb}MB file size")
