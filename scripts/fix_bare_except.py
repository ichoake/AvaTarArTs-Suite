...
class BareExceptFixer:
    ...
    def fix_file(self, filepath: Path, bare_excepts: List[Dict]) -> bool:
        ...
        try:
            ...
            print(f"{Colors.RED}Error fixing {filepath}: {e}{Colors.END}")
            return False
    ...
    def scan_and_fix(self):
        ...
        print(f"🔧 SCANNING FOR BARE EXCEPT CLAUSES")
        ...
        print(f"{Colors.GREEN}Scanning {len(python_files)} Python files...{Colors.END}\n")
        
        for idx, filepath in enumerate(python_files, 1):
            if idx % 100 == 0:
                print(f"{Colors.YELLOW}Progress: {idx}/{len(python_files)}...{Colors.END}", end='\r')
            ...
            if bare_excepts:
                ...
                print(f"   Found {len(bare_excepts)} bare except clause(s)")
                
                for except_info in bare_excepts:
                    print(f"   Line {except_info['line']}: {Colors.YELLOW}{except_info['original_line'].strip()}{Colors.END}")
                    print(f"   → Fix: {Colors.GREEN}except {except_info['suggested']}:{Colors.END}")
    ...
    def run(self):
        ...
        print("╔═══════════════════════════════════════════════════════════════════════════════╗")
        print("║                                                                               ║")
        print("║                    🔧 BARE EXCEPT FIXER 🧠                                    ║")
        print("║                                                                               ║")
        print("║              Intelligent Exception Handling Improvement                      ║")
        print("║                                                                               ║")
        print("╚═══════════════════════════════════════════════════════════════════════════════╝")
        print(f"{Colors.END}\n")
        ...
        if self.dry_run:
            print(f"{Colors.YELLOW}🛡️ MODE: DRY RUN (no files will be modified){Colors.END}\n")
        else:
            print(f"{Colors.RED}⚠️ MODE: LIVE FIX (files will be modified!){Colors.END}\n")
        ...
        print(f"  Files Scanned: {Colors.CYAN}{self.stats['files_scanned']:,}{Colors.END}")
        print(f"  Bare Excepts: {Colors.CYAN}{self.stats['bare_excepts_found']:,}{Colors.END}")
        print(f"  Files Fixed: {Colors.CYAN}{self.stats['files_fixed']:,}{Colors.END}")
        print(f"  Fixes Applied: {Colors.CYAN}{self.stats['fixes_applied']:,}{Colors.END}\n")
        
        if self.dry_run:
            print(f"{Colors.YELLOW}⚠️ DRY RUN complete. Use --live to apply fixes.{Colors.END}\n")
        else:
            print(f"{Colors.GREEN}✅ Fixes applied! Backups in: {self.backup_dir}{Colors.END}\n")
