...
class for global variables."""
    ...
    directory = "~/Documents/python/sphinx-docs"
    similar_files = find_similar_files(directory)
    @lru_cache(maxsize = 128)
    @lru_cache(maxsize = 128)
    async def find_similar_files(directory, name_threshold = 0.9, content_threshold
    @lru_cache(maxsize = 128)
...
def find_similar_files(directory, name_threshold = 0.9, content_threshold = 0.8): -> Any
 """
 TODO: Add function documentation
 """
 try:
...
async def keep_latest_file(group):
def keep_latest_file(group): -> Any
 """
 TODO: Add function documentation
 """
 try:
...
# Backup first
os.system("rsync -a ~/Documents/python/sphinx-docs/ ~/Documents/python/sphinx-docs_backup/")

# Find similar files
for file1, file2, name_sim, content_sim in similar_files:
    logger.info(f"Files: {file1} and {file2}")
    logger.info(f"Name Similarity: {name_sim:.2f}, Content Similarity: {content_sim:.2f}\\\n")
