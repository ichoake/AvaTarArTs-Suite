...
if not OPENAI_API_KEY:
    print(colored("❌ OPENAI_API_KEY missing. Ensure ~/.env or environment has it.", "red"))
    sys.exit(1)
...
def transcribe_audio(file_path: Path, model="whisper-1") -> str | None:
    if not file_path.is_file() or file_path.stat().st_size == 0:
        ...
        print(colored(f"❌ {file_path.name} invalid/empty.", "red"))
        return None
    ...
    try:
        ...
        print(colored(f"❌ Failed to transcribe {file_path.name}: {e}", "red"))
        return None
...
def analyze_text_for_section(text: str, model="gpt-3.5-turbo") -> str | None:
    ...
    try:
        ...
        print(colored(f"⚠️ Analysis error: {e}", "yellow"))
        return None
...
def process_audio_file(
    ...
    print(colored(f" Processing {audio_file.name}", "cyan"))

    if transcript_path.exists() and analysis_path.exists() and not force:
        print(colored(f"✔ Skipping {audio_file.name} (already done). Use --force to reprocess.", "green"))
        return

    transcript = transcribe_audio(audio_file)
    if not transcript:
        print(colored(f"⚠️ Skipping {audio_file.name} due to transcription failure.", "yellow"))
        return

    transcript_dir.mkdir(parents=True, exist_ok=True)
    transcript_path.write_text(transcript, encoding="utf-8")
    print(colored(f"✅ Transcript saved: {transcript_path.name}", "green"))

    transcript_segments = parse_transcript(transcript)

    analysis = analyze_text_for_section(transcript)
    if not analysis:
        print(colored(f"⚠️ Skipping analysis for {audio_file.name}.", "yellow"))
        return
    ...
    print(colored(f" Analysis saved: {analysis_path.name}", "green"))

def check_conda_env(expected="ai-media"):
    current = os.environ.get("CONDA_DEFAULT_ENV", "")
    if expected and expected not in current:
        print(colored(f"⚠️ You are in conda env '{current or 'none'}'; expected '{expected}'. Activate with: conda activate {expected}", "yellow"))
...
def process_audio_directory(audio_dir: Path, max_workers: int, force: bool):
    ...
    if not audio_files:
        print(colored("⚠️ No MP3s found in the directory.", "red"))
        return

    print(colored(f" Found {len(audio_files)} MP3 files. Starting...", "green"))
...
def main():
    ...
    parser.add_argument(
        ...
        "--audio-dir",
        type=str,
        default="/Users/steven/Music/nocTurneMeLoDieS/MP3/",
    ...
    if not audio_dir.is_dir():
        print(colored(f"❌ Audio directory invalid: {audio_dir}", "red"))
        sys.exit(1)
