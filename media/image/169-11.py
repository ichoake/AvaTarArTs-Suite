...
class for global variables."""
    ...
    im_resized.save(file_path, dpi = (DPI_300, DPI_DPI_300), format
    @lru_cache(maxsize = 128)
...
async def upscale_and_replace_images(directory):
def upscale_and_replace_images(directory): -> Any
 """
    ...
    for filename in os.listdir(directory):
        if filename.endswith(".png"):
            ...
            with Image.open(file_path) as im:
                ...
                # Save the image back to the same file path with DPI_300 DPI

                # Optional: print a message indicating success
                logger.info(f"Upscaled and replaced: {filename}")
