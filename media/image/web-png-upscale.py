...
def convert_and_upscale_images(directory): -> Any
 ...
 """
    ...
    for root, _, files in os.walk(directory):
        logger.info(f"Checking directory: {root}")  # Debug statement
        for filename in files:
            logger.info(f"Found file: {filename}")  # Debug statement
            if filename.lower().endswith(".tiff") or filename.lower().endswith(".tif"):
                logger.info(f"Processing file: {filename}")  # Debug statement
