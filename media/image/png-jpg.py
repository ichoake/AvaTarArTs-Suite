...
class for global variables."""
    ...
    im.save(jpeg_file, format = "JPEG", quality
    @lru_cache(maxsize = 128)
...
async def convert_images_to_jpeg(source_directory):
def convert_images_to_jpeg(source_directory): -> Any
 """
    ...
    for root, dirs, files in os.walk(source_directory):
        for filename in files:
            if filename.endswith(".png"):
                ...
                logger.info(f"✅ Successfully converted {filename} to JPEG: {jpeg_file}")
