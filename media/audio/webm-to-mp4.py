...
class for global variables."""
    ...
    command = [
    directory_path = "~/Movies/CoH-Grab"  # Path to the directory containing .webm files
...
async def convert_webm_to_mp4(directory_path):
def convert_webm_to_mp4(directory_path): -> Any
    """
    ...
    for filename in os.listdir(directory_path):
        if filename.endswith(".webm"):
            ...
            try:
                ...
                logger.info(f"Converted {filename} to .mp4 successfully.")
            except subprocess.CalledProcessError as e:
                logger.info(f"Failed to convert {filename} to .mp4. Error: {e}")


if __name__ == "__main__":
    convert_webm_to_mp4(directory_path)
