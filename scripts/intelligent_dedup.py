...
class IntelligentDeduplicator:
    ...
    def scan_codebase(self):
        ...
        print(f"{Colors.GREEN}Found {len(python_files)} Python files{Colors.END}\n")

        # Analyze each file
        for idx, filepath in enumerate(python_files, 1):
            if idx % 50 == 0:
                print(f"{Colors.YELLOW}Analyzing: {idx}/{len(python_files)}...{Colors.END}", end='\r')
    ...
    def create_removal_plan(self):
        ...
        print(f"{Colors.CYAN}Exact duplicate groups: {len(exact_dupes)}{Colors.END}")
        ...
        print(f"{Colors.CYAN}Semantic duplicate groups: {len(semantic_dupes)}{Colors.END}")
        print(f"{Colors.YELLOW}Total files to remove: {self.stats['files_removed']}{Colors.END}")
        print(f"{Colors.GREEN}Space to save: {self.stats['space_saved'] / 1024 / 1024:.2f} MB{Colors.END}")
    ...
    def execute_removal(self):
        ...
        print(f"{Colors.YELLOW}Total files to remove: {len(self.removal_plan)}{Colors.END}")
        print(f"{Colors.CYAN}Mode: {'DRY RUN' if self.dry_run else 'LIVE REMOVAL'}{Colors.END}\n")
        ...
        for idx, item in enumerate(self.removal_plan, 1):
            ...
            print(f"{Colors.RED}Remove: {remove_file.relative_to(self.target_dir)}{Colors.END}")
            print(f"  Parent: {Colors.CYAN}{item['remove_parent']}{Colors.END}")
            print(f"{Colors.GREEN}Keep:   {keep_file.relative_to(self.target_dir)}{Colors.END}")
            print(f"  Parent: {Colors.CYAN}{item['keep_parent']}{Colors.END}")
            ...
            if self.interactive:
                ...
                if 'backup' in str(remove_file).lower() or 'archive' in str(remove_file).lower():
                    ...
                    if choice == 'q':
                        print(f"{Colors.YELLOW}Removal aborted{Colors.END}")
                        return
                    proceed = choice == 'y'
            ...
            try:
                if not self.dry_run:
                    ...
                    # Remove file
                    remove_file.unlink()
                    print(f"{Colors.GREEN}{Emojis.CHECK} Removed{Colors.END}")
                else:
                    print(f"{Colors.YELLOW}[DRY RUN] Would remove{Colors.END}")

            except Exception as e:
                print(f"{Colors.RED}{Emojis.WARN} Error: {e}{Colors.END}")
    ...
    def generate_report(self):
        ...
        with open(report_file, 'w') as f:
            ...
            f.write(f"| Space Saved | {self.stats['space_saved'] / 1024 / 1024:.2f} MB |\n")
            ...
            for parent in sorted(by_parent.keys()):
                ...
                f.write(f"### 📁 {parent}\n")
                f.write(f"**Removed:** {len(items)} files ({total_size / 1024:.1f} KB)\n\n")
        ...
        print(f"{Colors.GREEN}{Emojis.CHECK} Report: {report_file}{Colors.END}")
        print(f"{Colors.GREEN}{Emojis.CHECK} Data: {json_file}{Colors.END}")
        if undo_script.exists():
            print(f"{Colors.GREEN}{Emojis.CHECK} Undo: {undo_script}{Colors.END}")
    ...
    def run(self):
        ...
        print("║                                                                               ║")
        print("║        Content-Aware Deduplication with Parent Folder Intelligence           ║")
        print("║                                                                               ║")
        print("╚═══════════════════════════════════════════════════════════════════════════════╝")
        print(f"{Colors.END}\n")
        ...
        if self.dry_run:
            print(f"{Colors.YELLOW}{Emojis.SHIELD} MODE: DRY RUN (no files will be removed){Colors.END}")
        else:
            print(f"{Colors.RED}{Emojis.WARN} MODE: LIVE REMOVAL (files will be deleted!){Colors.END}")

        print(f"{Colors.CYAN}Interactive: {'Yes' if self.interactive else 'No'}{Colors.END}\n")
        ...
        print(f"  {Emojis.FILE} Scanned: {Colors.CYAN}{self.stats['total_files']:,}{Colors.END}")
        print(f"  {Emojis.TARGET} Exact Dupes: {Colors.CYAN}{self.stats['exact_duplicates']:,} groups{Colors.END}")
        print(f"  {Emojis.BRAIN} Semantic Dupes: {Colors.CYAN}{self.stats['semantic_duplicates']:,} groups{Colors.END}")
        print(f"  {Emojis.TRASH} Removed: {Colors.CYAN}{self.stats['files_removed']:,} files{Colors.END}")
        print(f"  💾 Space Saved: {Colors.CYAN}{self.stats['space_saved'] / 1024 / 1024:.2f} MB{Colors.END}\n")

        if self.dry_run:
            print(f"{Colors.YELLOW}{Emojis.WARN} This was a DRY RUN. Use --live to actually remove files.{Colors.END}\n")
        else:
            print(f"{Colors.GREEN}{Emojis.CHECK} Deduplication complete!{Colors.END}\n")
            print(f"{Colors.CYAN}Backups saved to: {self.backup_dir}{Colors.END}\n")
...
def main():
    ...
    parser.add_argument('--target', type=str,
                       default="/Users/steven/GitHub/AvaTarArTs-Suite",
