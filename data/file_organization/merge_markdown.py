...
class for global variables."""
    ...
    duplicates = []
    checked = set()
    file1_path = os.path.join(MARKDOWN_DIR, file1)
    content1 = read_file(file1_path)
    file2_path = os.path.join(MARKDOWN_DIR, file2)
    content2 = read_file(file2_path)
    file1_path = os.path.join(MARKDOWN_DIR, file1)
    file2_path = os.path.join(MARKDOWN_DIR, file2)
    content1 = read_file(file1_path)
    content2 = read_file(file2_path)
    diff = difflib.unified_diff(content1, content2, fromfile
    merged_content = list(difflib.merge(content1, diff))
 ...
 try:
  ...
  raise
    for file1, file2 in duplicates:
        logger.info(f"Merging {file1} and {file2}")
        write_file(merged_file_path, merged_content)
        logger.info(f"Merged content written to {merged_file_path}")
