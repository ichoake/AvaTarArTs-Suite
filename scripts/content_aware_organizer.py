...
class ContentAwareAnalyzer:
    ...
    CATEGORY_PATTERNS = {
        'ai_tools': ['ai', 'ml', 'model', 'neural', 'gpt', 'claude', 'gemini', 'openai'],
...
class ParentFolderStructureAnalyzer:
    ...
    def build_folder_tree(self, current_path: Path, parent_node: FolderNode, current_depth: int = 0):
        ...
        skip_dirs = {'.git', '__pycache__', 'node_modules', '.venv', 'venv', 
                     'merge_backup_', 'merge_analysis_', 'dedup_backup', 'ai_diff_reports_'}
    ...
    def generate_folder_structure_report(self):
        ...
        with open(report_file, 'w') as f:
            ...
            for category, count in sorted(self.stats['categories_found'].items(), 
            ...
            for category, folders in sorted(self.category_folders.items()):
                if folders:
                    ...
                    for folder in sorted(folders, key=lambda x: len(x.files), reverse=True)[:10]:
                        ...
                        f.write(f"- **Size:** {folder.total_size / 1024:.1f} KB\n")
    ...
    def run(self):
        ...
        print(f"{Colors.GREEN}Built tree with {self.stats['total_folders']} folders, "
        ...
        print(f"{Colors.BOLD}🎯 TOP CATEGORIES:{Colors.END}\n")
        for category, count in sorted(self.stats['categories_found'].items(), 
                                     key=lambda x: x[1], reverse=True)[:5]:
            print(f"  {Colors.CYAN}{category}{Colors.END}: {count} folders")
