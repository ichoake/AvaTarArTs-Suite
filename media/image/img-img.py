...
async def convert_and_upscale_images_in_subfolders(source_directory):
def convert_and_upscale_images_in_subfolders(source_directory): -> Any
 """
    ...
    for root, dirs, files in os.walk(source_directory):
        for filename in files:
            if filename.endswith(".png"):
                ...
                logger.info(f"🔄 Upscaling {filename} to: {upscale_width}x{upscale_height}")
                ...
                logger.info(f"✅ Successfully converted and upscaled: {filename}")
...
def main(): -> Any
 ...
 try:
  ...
  raise
    ...
    convert_and_upscale_images_in_subfolders(source_directory)
